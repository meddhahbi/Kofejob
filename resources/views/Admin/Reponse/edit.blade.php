...
<div class="page-wrapper">
    <div class="content container-fluid">
        ...
        <form action="{{ route('Admin.Reponse.update', ['id' => $reponse->id]) }}" method="POST">
            @csrf
         
            <div class="form-group">
                <label for="contenu">Contenu de la Réponse</label>
                <textarea class="form-control" id="contenu" name="contenu" rows="5" required>{{ $reponse->contenu }}</textarea>
            </div>
            <div class="form-group">
                ...
                <select class="form-control" id="condidat_id" name="condidat_id" required>
                    @foreach ($condidats as $condidat)
                        <option value="{{ $condidat->id }}" {{ $reponse->condidat_id == $condidat->id ? 'selected' : '' }}>
